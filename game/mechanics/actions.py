...
def move(game_instance, action_data: dict) -> dict:
    """Make hero move"""
    destination = game_instance.board.hexes.get(action_data['destination'], None)
    if not destination:
        return {'allowed': False}
    hero_hex = game_instance.board.hexes[game_instance.game.hero.position]
    if game_instance.board.distance(destination, hero_hex) != 1:
        return {'allowed': False}
    if destination['occupied_by'] != 'empty':
        return {'allowed': False}

    hero_hex['occupied_by'] = 'empty'
    game_instance.game.hero.position = f"{destination['q']};{destination['r']}"
...
def attack(game_instance, source: BaseUnit, target: BaseUnit) -> dict:
    """Simple melee attack"""
    hero_hex = game_instance.board.hexes[source.position]
    target_hex = game_instance.board.hexes[target.position]
